...
def train_and_save(
    ...
    try:
        ...
        print(f"Error downloading model {encoder_name}: {e}")
        print("Using fallback TF-IDF approach...")
        # Fallback to TF-IDF if SBERT fails
        from sklearn.feature_extraction.text import TfidfVectorizer
        encoder = TfidfVectorizer(max_features=1000, stop_words='english')
        X_emb = encoder.fit_transform(X_texts).toarray()
        # Continue with TF-IDF approach
    ...
    # Save model data based on encoder type
    if encoder_name == 'tfidf':
        model_data = {
            "encoder_name": encoder_name,
            "vectorizador": encoder,
            "clasificador": clf,
        }
    else:
        model_data = {
